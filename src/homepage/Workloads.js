			...
			return (
				<Card key={key} className="p-3" container="container-sm"
					...
					<Card.Title>
						...
						/>
						POD {parseInt(key)}
						<span id={"podMembNum" + parseInt(key)} className={"float-right"}
						      style={{fontSize: "15px"}}>Total Members: {podMem}</span>
					</Card.Title>
		...
		<div>
			<QHNavBar
				loading={(workloadLoading || expRatioLoading || clientsConfigLoading
					...
					|| memberLoading || clientPSRLoading)} cardsRefsMap={cardsRefsMap}/>
			<div>
